...
import 'package:team_project_front/features/report/component/custom_text_input.dart';
import 'package:team_project_front/features/report/model/report_info.dart';
import 'package:team_project_front/features/report/view/create_report.dart';
...
class ChangeReport extends StatefulWidget {
  ...
  const ChangeReport({super.key, required this.report});
...
class _ChangeReportState extends State<ChangeReport> {
  ...
  void initialize() async {
    ...
    setState(() {
      ...
      outingController = TextEditingController(
        text: widget.report.outingRecord,
      );
  ...
  void onSavePressed() async {
    ...
    try {
      ...
      if (response.statusCode == 200 && resData['isSuccess'] == true) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('리포트 수정이 완료되었습니다')));
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('리포트 수정 중 오류가 발생했어요')));
    }
  }
